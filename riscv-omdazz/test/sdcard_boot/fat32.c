...
static int readsect(fat32_t *fs, unsigned int number)
{
	return fs->read(fs->user, number, fs->buffer);
}

int fat32_mount(fat32_t *fs, int (*read_sector)(void *user, unsigned int number, void *buffer), void *user, int partition_start)
{
	fat32_boot_t *boot;
