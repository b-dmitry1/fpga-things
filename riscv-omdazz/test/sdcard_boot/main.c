...
int main(void)
{
	...
	fat32_entry_t *dir = (fat32_entry_t *)buf;
...
init_loop:
	...
	if (!fat32_mount(&fat32, read_sector, 0, part_start))
	{
		print("mount failed\n");
		for (;;);
	}

	print_int("mount ok\nFAT start: ", fat32.fat_start, "\n");
	print_int("Root dir start: ", fat32.root_dir_start, "\n");

	sd_read(buf, fat32.root_dir_start);
